#!/usr/bin/env python3
"""
End-to-End Load Test with 500 Phone Numbers

Tests the complete RCS platform flow under load:
1. Creates template, audience with 500 contacts
...
Usage:
    ...
    python tests/test_e2e_500_numbers.py
"""
...
async def main():
    """Run end-to-end load test with 500 numbers"""
    
    print_header("🧪 RCS PLATFORM - END-TO-END LOAD TEST (500 NUMBERS)", "═")
    ...
    setup_logging(
        log_level="INFO",
        log_file="logs/test_e2e_500_numbers.log"
    )
    print(f"\n📝 Logging to: logs/test_e2e_500_numbers.log")
    ...
    try:
        ...
        async with db.session() as session:
            ...
            template = Template.create(
                tenant_id=tenant_id,
                name="E2E Load Test Template - 500 Numbers",
            ...
            # Mock: auto-approve
            if use_mock:
                template.approve_template("mock-template-e2e-500")
        ...
        # ──────────────────────────────────────────────────────────────────
        # STEP 3: Create Audience with 500 contacts
        # ──────────────────────────────────────────────────────────────────
        print_section("STEP 3: Creating Audience with 500 Test Contacts")
        
        # Generate 500 test phone numbers
        test_contacts = []
        for i in range(500):
            phone = f"+91987654{i:04d}"  # +919876540000 to +919876540499
            contact = Contact(
                phone_number=phone,
                metadata={},
            )
            # Ordered list matching template placeholders: {{name}}, {{test_id}}
            contact.variables = [f"Test User {i+1}", str(i+1)]
        ...
        async with db.session() as session:
            ...
            audience = Audience.create(
                tenant_id=tenant_id,
                name="E2E Load Test Audience - 500 Numbers",
            )
            ...
            for i, contact in enumerate(test_contacts[:5], 1):
                print(f"      {i}. {contact.phone_number} - variables={contact.variables}")
        ...
        async with db.session() as session:
            ...
            campaign = await campaign_service.create_campaign(
                tenant_id=tenant_id,
                name="E2E Load Test Campaign - 500 Numbers",
        ...
        success = await wait_for_processing(
            ...
            timeout_seconds=600  # 10 minutes for 500 messages
        )
        ...
        if success:
            ...
            print(f"   View logs: logs/test_e2e_500_numbers.log")
            return 0
        else:
            print("\n⚠️  Processing incomplete or timed out")
            print("   Check:")
            print("   1. Are workers running?")
            print("   2. Check logs/test_e2e_500_numbers.log")
